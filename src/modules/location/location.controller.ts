...
const getSingleByIdFromDB = catchAsync (async (req:Request & { user?: IJWTPayload }, res:Response) =>{
   const {availabilityId} = req.params; 
    ...
    const result = await LocationService.getSingleByIdFromDB(req.user as IJWTPayload, availabilityId);
    console.log("result", result);
...
const updateIntoDB = catchAsync (async (req:Request & { user?: IJWTPayload }, res:Response) =>{
  ...
  const {startAt, endAt}= req.body;
    const result = await LocationService.updateIntoDB(user as IJWTPayload , availabilityId, new Date(startAt), new Date(endAt));
    console.log("result", result);
...
const deleteFromDB = catchAsync (async (req:Request , res:Response) =>{
  const categoryId = req.params.categoryId;
    const result = await LocationService.deleteFromDB(categoryId);
